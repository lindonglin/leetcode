...
public class Rob {
    ...
    @Test
    public void case1() {
        TreeNode root = TreeNode.fullOrder(new Integer[]{3, 2, 3, null, 3, null, 1});
        int expected = 7;
        Assert.assertEquals(expected, rob(root));
    }

    @Test
    public void case2() {
        TreeNode root = TreeNode.fullOrder(new Integer[]{3, 4, 5, 1, 3, null, 1});
        int expected = 9;
        Assert.assertEquals(expected, rob(root));
    }
    ...
    private int[] invoke(TreeNode root) {
        if (root == null) {
            return new int[]{0, 0};
        }
