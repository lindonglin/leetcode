...
public class Bipartite {
    ...
    @Test
    public void case6() {
        int[][] graph = {{}, {2, 4, 6}, {1, 4, 8, 9}, {7, 8}, {1, 2, 8, 9}, {3, 6, 9}, {2, 3, 4, 6, 9}, {2, 4, 5, 6, 7, 8}};
        Assert.assertFalse(isBipartite(graph));
    }
    ...
    private boolean isBipartite(int[][] graph) {
        ...
        for (int i = 0; i < n; i++) {
            if (groups[i] == 0) {
                groups[i] = 'A';
                if (notBipartite(groups, i, graph)) {
                    return false;
                }
            }
        }
    ...
    private boolean notBipartite(char[] groups, int i, int[][] graph) {
        char group = groups[i] == 'A' ? 'B' : 'A';
        for (int neighbor : graph[i]) {
            if (groups[neighbor] == 0) {
                groups[neighbor] = group;
                if (notBipartite(groups, neighbor, graph)) {
                    return true;
                }
